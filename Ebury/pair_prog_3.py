class LoanLedger:
    ...
    def register_loan(self, loan_id, borrower_id, credit_limit):
        ...
        if borrower_id not in self.borrower_loans:
            ...
            self.borrower_limits[borrower_id] = credit_limit
    ...
    def add_transaction(self, loan_id, transaction_type, amount):
        ...
        if transaction_type == "DISBURSEMENT":
            ...
            if new_exposure > self.borrower_limits[borrower_id]:
    ...
    def get_borrower_exposure(self, borrower_id):
        return sum(
            self.loan_balances[loan_id]
            for loan_id in self.borrower_loans.get(borrower_id, [])
        )

    def get_available_credit(self, borrower_id):
        return(
            self.borrower_limits[borrower_id] - self.get_borrower_exposure(borrower_id) 
        )
