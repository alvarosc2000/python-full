class LoanLedger:
    ...
    def add_transaction(self, load_id, transaction_type, amount):
        if load_id not in self.loan_balances:
            self.loan_balances[load_id] = 0
            
        if transaction_type == "DISBURSEMENT":
            self.loan_balances[load_id] += amount
            
        elif transaction_type == "REPAYMENT":
            self.loan_balances[load_id] -= amount
    
    def get_balance(self,load_id):
        return self.loan_balances.get(load_id,0)
